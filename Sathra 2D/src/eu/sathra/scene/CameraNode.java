...
public class CameraNode extends SceneNode {
	...
	@Deserialize({ Deserialize.NULL, "id", "x", "y", "is_visible", "animation",
			"children", "body", "ai" })
	@Defaults({ Deserialize.NULL, Deserialize.NULL, Deserialize.NULL,
			Deserialize.NULL, "true", Deserialize.NULL, Deserialize.NULL,
			Deserialize.NULL, Deserialize.NULL })
	public CameraNode(SathraActivity sathra, String id, float x, float y,
	...
	protected void draw(GL10 gl, long time, long delta) {
		...
		float x2 = (getAbsoluteX() + mSathra.getScreenWidth()) - mSathra.getScreenWidth() / 2;
		float y2 = (getAbsoluteY() + mSathra.getScreenHeight() ) - mSathra.getScreenHeight() / 2;
